...
export default class Manhattan extends HTMLElement{
    constructor(){
        super();
    }
    ...
    connectedCallback(){
        ...
        // Draw line
        svg.append(createSVGElement("line", {"x1":-1*cellSize, "y1":0*cellSize, "x2":0*cellSize, "y2":1*cellSize, "stroke":"red",
        ...
        figcaption.textContent = String.raw`All vectors on the red outline above all have a Manhattan norm of 1`;
        ...
        svg.addEventListener("mousemove", e=>{
            ...
            if(Math.abs(x) < 1){
                x = Math.sign(x)*(1 - Math.abs(y))
            }
            else{
                y = Math.sign(y)*(1 - Math.abs(x))
            }
...
customElements.define("mf-manhattan", Manhattan);